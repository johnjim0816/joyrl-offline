...
class Main(object):
    ...
    def create_dirs(self, cfg):
        ...
        task_dir = f"{curr_path}/tasks/{cfg.mode.capitalize()}_{cfg.backend}_{cfg.env_name}_{cfg.algo_name}_{curr_time}"
    ...
    def run(self) -> None:
        ...
        if cfg.backend == 'single' and cfg.n_workers == 1:
            ...
            raise ValueError(f'The backend you set is {cfg.backend}, but it must be single, multiprocessing or ray')
...
if __name__ == "__main__":
    ...
    main.run()
    end_t = time.time()
    Elapsing_time = end_t - start_t
    print(f'Elapsing time:{Elapsing_time}')
