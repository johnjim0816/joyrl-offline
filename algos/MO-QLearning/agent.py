...
class Agent(object):
    ...
    # def scalarized_q_values(self, state) -> np.ndarray:
    #     """Returns the scalarized Q values for each action, given stateervation and weights."""
    #     t_state = tuple(state)
    #     if t_state not in self.Q_table:
    #         return np.zeros(self.action_dim)
    #     return np.array([np.dot(state_action_value, self.weights) for state_action_value in self.Q_table[t_state]])

    #     np.dot(state_action_value, w)
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        if done: # terminal state
            Q_target = np.dot(reward, self.weights)  
        else:
            Q_target = np.dot(reward, self.weights) + self.gamma * np.max(self.Q_table[str(next_state)]) 
    ...
    def save_model(self,path):
        ...
        torch.save(
            obj=self.Q_table,
            f=path+"MO-Qleaning_model.pkl",
            pickle_module=dill
        )
        print("Model saved!")
    def load_model(self, path):
        import dill
        self.Q_table =torch.load(f=path+'MO-Qleaning_model.pkl',pickle_module=dill)
        print("Mode loaded!")