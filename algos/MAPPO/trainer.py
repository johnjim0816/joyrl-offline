...
from algos.MAPPO.config import AlgoConfig
from algos.MAPPO.envs import DummyVecEnv
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            from envs import ContinuousActionEnv

            env = ContinuousActionEnv()
            # from envs import DiscreteActionEnv
...
def parse_args(parser):
    parser.scenario_name = 'MyEnv'
    parser.num_landmarks = 3
    parser.num_agents = 2
...
def main(args):
    ...
    # run dir
    run_dir = (
        Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[0] + "/MAPPO_results")
        # / all_args.env_name
    ...
    if all_args.share_policy:
        from env_runner import Shared_EnvRunner as Runner
    else:
        from env_runner import Sep_Runner as Runner
