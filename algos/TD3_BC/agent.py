...
class Agent(object):
	def __init__(self,cfg):
		...
		if cfg.normalize:
			self.mean, self.std, self.expert_states, self.expert_next_states = self.normalize_states(self.expert_states, self.expert_next_states) 
		else:
	...
	def update(self, state, action, next_state, reward, done):
		...
		if self.sample_count % self.policy_freq == 0:
			...
			probs_scale = self.action_scale * probs + self.action_bias
			q_critic = self.critic_1(torch.cat([state, probs_scale], 1))
			# with torch.no_grad():
			lmbda = self.alpha/q_critic.abs().mean().detach()
			# TD3 + BC
			actor_loss = - lmbda * q_critic.mean() + F.mse_loss(probs_scale, action) 
