...
LastEditTime: 2023-04-15 21:38:14
Discription: 
'''
import torch
    ...
    def train(self):
        while self.global_ep <= (self.train_eps):
            ...
            while True:
                ...
                share_agent_policy_state_dict= ray.get(self.share_agent.get_parameters.remote())
                # update share_agent
                share_agent_policy_state_dict = self.local_agent.update_ray(share_agent_policy_state_dict)
                # return share_agent to ShareAent
                ray.get(self.share_agent.receive_parameters.remote(share_agent_policy_state_dict))
                state = next_state
                ep_r += reward
                if terminated or ep_step >= self.max_steps:
